    ...
    const departmentsArray = [
     ...
     useEffect(()=>{
        const fetchDoctors = async()=>{
            const {data} = await axios.get("http://localhost:4000/api/v1/doctor",{withCredentials:true});
...
<h2>Appointment</h2>
<form onSumbit= {handleAppointment}>

 <div>
    <input type="text" placeholder="First Name" value= {firstName} onChnage={(e)=> setFirstName(e.target.value)}/>
    <input type="text" placeholder="Last Name" value = {lastName} onChange= {(e)=>setLastName(e.target.value)}/>
 </div>
 <div>
    <input type="text" placeholder="Email" value= {email} onChnage={(e)=> setEmail(e.target.value)}/>
    <input type="number" placeholder="Phone Number" value = {phone} onChange= {(e)=>setPhone(e.target.value)}/>

 </div>
 <div>
    <input type="number" placeholder="NIC" value= {nic} onChnage={(e)=> setNic(e.target.value)}/>
 ...
 <div>
   ...
   <select value= {`${doctorFirstName} ${doctorLastName}`} onChange={(e)=>{
    ...
    {doctors.filter((doctors)=>doctors.doctordepartment === department).map((doctor,index)=>{
...
{/* <input type="radio" name="visited" value="Yes" onChange={(e)=> setHasVisited(e.target.value)}/> */}
{/* <Link to={"/login"} style={{textDecoration:"none",alignItems:"center"}}>Login Now</Link> */}

</div>
<div style={{justifyContent:"center",alignItems:"center"}}>
  <button type="submit">GET APPOITNMENT</button>  
</div>
